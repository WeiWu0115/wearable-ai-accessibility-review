...
1. **★ 各感官模态"出现 / 爆发"时间线(signature 图)** —— papers/year 按模态分层。charting 已记 `年份 × 模态`,**编码即副产品**。
...
3. 建 charting 模板表(字段:title / authors / year / venue / 技术 / 模态 / 目标层 / 神经调用类型·承重 / augment-substitute-regulate / 目标人群 / 扩散定位 / 关键发现)。
